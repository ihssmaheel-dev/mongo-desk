                  ...
                  <div class="relative">
                    ...
                    {#if activeFilterCol === col}
                      ...
                      <div class="absolute top-full left-0 z-20 mt-1 w-56 rounded-lg border border-[#2D3A45] bg-[#1F2933] shadow-xl p-2">
                        ...
                        <select class="mb-2 w-full rounded border border-[#2D3A45] bg-[#0E1318] px-2 py-1 text-[11px] text-[#C3D4DE] outline-none" value={filters[col]?.op || 'eq'} onchange={(e) => { const val = (e.target as HTMLSelectElement).value; filters = { ...filters, [col]: { op: val, value: filters[col]?.value || '' }}; }}>
                          {#each ops as op}
